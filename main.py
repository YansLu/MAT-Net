...
def plot_confusion_matrix(cm, labels_name, title, acc):
    ...
    plt.xticks(num_class, labels_name, rotation=45)  # 将标签印在x轴坐标上
    plt.yticks(num_class, labels_name)  # 将标签印在y轴坐标上
    # plt.ylabel('Target')
    # plt.xlabel('Prediction')
    plt.imshow(cm, interpolation='nearest', cmap=plt.get_cmap('Blues'))
    plt.tight_layout()
    plt.savefig(os.path.join('Confusion_matrix/raf-db/MDR/MDR-Net', "acc" + str(acc) + ".png"), format='png')
    # plt.savefig(os.path.join('./Confusion_matrix/raf-db/Pose_occ', "Confusion Matrix of RAF-DB Dataset(occ)"+ ".jpg"), format='png')# raf-db
    # Confusion Matrix of RAF-DB Dataset
    plt.show()
...
parser.add_argument('--data', type=str, default='./data/raf-db')
...
parser.add_argument('-e', '--evaluate', default=None, type=str, help='evaluate model on test set') # './models/RAF_MAT/MDR/epoch20_acc98.72881.pth'
parser.add_argument('-pc', '--plot_cm', default=True, action="store_true", help="Ploting confusion matrix.")
...
# RAF-baseline'./models/RAF_MAT/MAT_ir50/Baseline/epoch8_acc91.0691.pth'
# RAF MAT ./models/RAF_MAT/MAT_ir50/MAT/epoch8_acc95.17601.pth
#
gpus = [0,1,2,3]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            ...
            save_checkpoint({'epoch': epoch + 1,
                             ...
                             'recorder1': recorder1,
                             'recorder': recorder}, epoch + 1, best_acc, args)
            '''
...
def validate(val_loader, model, criterion, args):
    ...
    if args.plot_cm:
        ...
        # labels_name = ['NE', 'HA', 'SA', 'SU', 'FE', 'DI', "AN"]  # 横纵坐标标签
        labels_name = ['Neutral', 'Happiness', 'Sadness', 'Surprise', 'Fear', 'Disgust', "Anger"]
        plt.clf()
        plot_confusion_matrix(cm, labels_name, 'Confusion Matrix of RAF-DB Dataset', acc)

    return top1.avg, losses.avg, output, target, D


def save_checkpoint(state, epoch, best_acc, args):
    Acc = best_acc.cpu().numpy()
    torch.save(state, os.path.join('models/RAF_MAT/MDR/MDR-Net', "epoch" + str(epoch) + "_acc" + str(Acc) + ".pth"))
...
class RecorderMeter1(object):
    ...
    def reset(self, total_epoch):
        ...
        self.epoch_losses = np.zeros((self.total_epoch, 2), dtype=np.float32)  # [epoch, train/val_occ]
        self.epoch_accuracy = np.zeros((self.total_epoch, 2), dtype=np.float32)  # [epoch, train/val_occ]

    def update(self, output, target):
        self.y_pred = output
        self.y_true = target

    def plot_confusion_matrix(self, cm, title='Confusion_matrix Matrix', cmap=plt.cm.binary):
...
class RecorderMeter(object):
    ...
    def plot_curve(self, save_path):
        title = 'the accuracy/loss curve of train/val_occ'
        dpi = 80
